...
from meet1 import agent_cancel_executor, agent_reschedule_executor, agent_schedule_executor
from policy1 import agent_policy_executor
import logging

# Setup basic logging (uncomment and adjust if desired)
# logging.basicConfig(filename='app.log', level=logging.INFO,
#                     format='%(asctime)s - %(levelname)s - %(message)s')

app = Flask(__name__)

# Global variables to store the last processed email ID and response
last_processed_email_id = None
processed_response = {}  # This will hold the details of the last processed email
...
def send_email_reply(service, original_email, reply_body, intent):
    ...
    try:
        ...
        # Check intent and format reply accordingly using templates
    ...
    while True:
        try:
            ...
            if latest_email:
                ...
                # Update the global processed_response with the details
                processed_response = {
...
def fetch_and_classify_email():
    """
    Return the last processed email response.
    (Since processing is handled by the background thread, this endpoint simply
    returns the last stored response.)
    """
    global processed_response
    try:
        if processed_response:
            # Return a copy of the processed response
            response = processed_response.copy()
            return jsonify(response)
        else:
            return jsonify({"error": "No email processed yet."}), 404
    except Exception as e:
